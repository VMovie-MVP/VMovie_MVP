...
public class HotAdapter extends BaseLvAdapter<ChanelDetailBean> {

    public HotAdapter(List<ChanelDetailBean> data, Context context, String viewType, int... layoutIds) {
        super(data, context, viewType, layoutIds);
    }

    @Override
    protected void bindData(ViewHolder holder, ChanelDetailBean item, int position) {
        holder.setText(R.id.chanel_detail_lv_item1_title,item.getWx_small_app_title());
